                ...
                try:
                        ...
                        if len(Hum)==10:
                                ...
                                if temp is not None and hum is not None:
                                        if settings.Line_on:
                                                THI_cal(THI)
                                        if settings.NTU_db_enable==1:
                                                try:
                                                        # NTU - connect to the MySQL server and insert data
                                                        ...
                                                        conn.commit()
                                                        conn.close()
                                                        print("Insert data into NTU MySQL successfully")
                                        ...
                                        if settings.III_db_settings==1:
                                                try:
                                                        ...
                                                        data_post = requests.post(settings.III_server+record_dt+THI_str+info)
                                                        print(data_post.text)
                                                except:
                                                        pass
                                        if settings.SF_db_enable==1:
                                                try:
                                                        conn = pymssql.connect(host=settings.sf_host, user=settings.sf_user, password=settings.sf_passwd, database=settings.sf_db)
                                                        x = conn.cursor()
                                                        action = 'INSERT INTO '+ settings.sf_table +' (DATE,T,H,THI,FARM,NODE,EXPT,CREATE_DATE,CREATE_USER,UPDATE_DATE,UPDATE_USER,IS_ACTIVE) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                                                        data_value = (date_stamp,temp,hum,THI,"Fourways",node,settings.exp,date_stamp,"NTU",date_stamp,"NTU","Y")
                                                        x.execute(action ,data_value)
                                                        conn.commit()
                                                        conn.close()
                                                        print("Insert data into Fourways(SF) MSSQL successfully")
                                                except:
                                                        pass
                except:
                        pass
