    ...
    for(i,f) in enumerate(stream):
        ...
        if pre_inout == 1 and now_inout ==1 and inout_flag == 0:
            ...
            if settings.III_db_settings==1:
            	try:
            		record_dt = ('?drink_sdt='+start_date_stamp+'&drink_edt='+end_date_stamp)
            		info = ('&ranch_no='+node+'&ranch_name='+settings.farm)
            		print(record_dt+info)
            		data_post = requests.post("http://220.135.180.160:80/CMS/api/sensor/ranchDrink"+record_dt+info)
