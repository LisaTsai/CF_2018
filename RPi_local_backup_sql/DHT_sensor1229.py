...
#
#   Log:
#   12/27/2018 - Add the backup server
#   09/17/2018 - Send THI data to two databases in 2 ways
					...
					if db_enable==1:
						try:
							if s1==1:
								...
								x=conn.cursor()
								x.execute('INSERT INTO pn_cf_thi (DATE,T,H,THI,FARM,EXPT,NODE)' 'VALUES (%s,%s,%s,%s,%s,%s,%s)',(date_stamp,temp,hum,THI,location,"01",node))
								conn.commit()
								conn.close()
						except:
							pass
						try:
								...
								cur=db.cursor()
								cur.execute("""DELETE FROM THI where node = '9'""")
								cur.execute('INSERT INTO THI (datetime,T,H,THI,node)' 'VALUES(CURRENT_TIMESTAMP,%s,%s,%s,%s)',(temp,hum,THI,node))
								db.commit()
								db.close()
						except:
							pass
